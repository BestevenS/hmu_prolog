...
pushElementsToQueue([Elem|Rest], Q1, FQ):- 
	enqueue(Elem, Q1, NQ),
	pushElementsToQueue(Rest, NQ, FQ).
...
add_list_to_queue(Child, Q1, NQ):-
	add_list_to_queue1(Child, Q1, NQ).
...
print_path([Kat, Par], S1):-
	member_set([Par, Par2], S1),
	print_path([Par, Par2], S1),
	write('--> '),
	write(Kat).

get_children(Kat, Rest, S1, Children):-
	bagof(Child, moves(Kat, Rest, S1, Child), Children).
	
get_children(Kat,Rest,Closed_set,[]).
moves(Kat,Rest,S1, [Next,Kat]):-
	move(Kat, Next),
	\+(member([Next,Node], Rest)),
	\+(member_set([Next,_NodeY], S1)).
												  
% BFS

breadthFirstSearch(Arxi, Goal):-
	empty_queue(EmptyQueue),
	enqueue([Arxi, met], EmptyQueue, Q2),
	empty_set(S1),
	path(Q2, S1, Goal).
...
path(Q2, S1, Goal):-
	dequeue([Kat, Par], Q2, NQ2),
	goal(Kat),
	Kat = Goal,
	write('monopati lisis:'), nl,
	print_path([Kat, Par], S1).

path(Q2, S1, Goal):-
	dequeue([Kat, Par], Q2, Rest), 
	get_children(Kat, Rest,S1,Children),
